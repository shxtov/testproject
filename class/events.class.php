...
class Events{
    ...
    function getAllEvents ($q, $sort, $order){
        ...
        if ($q == ""){
            $stmt = $this->connection->prepare("SELECT id, eventType, eventDate, eventPrice, eventDescr FROM events_archive WHERE deleted is NULL ORDER BY $sort $orderBy");
        }else{
            $stmt = $this->connection->prepare("SELECT id, eventType, eventDate, eventPrice, eventDescr FROM events_archive WHERE deleted is NULL AND (eventType LIKE ? OR eventDate LIKE ? OR eventPrice LIKE ? OR eventDescr LIKE ?) ORDER BY $sort $orderBy");
            $searchWord = "%".$q."%";
            $stmt->bind_param("ssds", $searchWord, $searchWord, $searchWord, $searchWord);
        }
